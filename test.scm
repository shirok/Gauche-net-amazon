...
(define *test-mode* 'direct) ;; 'record, 'replay or 'direct
(define *load-test* #t)
...
;; if record or replay, this function returns the dummy server process-id
(define (run-server-by-mode)
  (and (member *test-mode* '(record replay))
       (let1 *test-port* 8080
         (with-module net.amazon.s3
           ...
           (set! *s3-endpoint* #`"localhost:,|*test-port*|"))
         (run-dummy-server *test-port* *test-mode*)
         )))

(sys-srandom (sys-time))
(let ((bucket #`",(sys-random)")
      (dummy-server (run-server-by-mode)))
...
Server: AmazonS3"
              ...
              (when *load-test*
                (let1 bucket #`",(sys-random)"
                  (s3-bucket-create! bucket)
                  (dotimes (cnt2 24)
                    (print cnt2)
                    (dotimes (cnt 24 1)
                      (let1 etag (s3-object-put! bucket "test-obj.txt"
                                                 (make-string (expt 2 cnt) #\x))
                        (test* #`"upload ,(expt 2 cnt)(2^,|cnt|)bytes data" etag
                               (rfc822-header-ref (s3-object-head bucket "test-obj.txt")
                                                  "etag")))))
                  (dotimes (cnt 16)
                    (let* ((upload-content (make-string (expt 2 cnt) #\x))
                           (etag (s3-object-put! bucket "test-obj.txt"
                                                 upload-content)))
                      (test* #`"upload content ,(expt 2 cnt)(2^,|cnt|)bytes data" upload-content
                             (s3-object-get bucket "test-obj.txt"))))
                  (s3-object-delete! bucket "test-obj.txt")
                  (s3-bucket-delete! bucket)))
              ))))
      (lambda ()
        (when dummy-server (sys-kill dummy-server SIGHUP)))))
